...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 현금 | 8% | **12%** | **33%** | **30%** | 15% | 50% | **27%** | **31%** |
| 나스닥 | 32% | **38%** | **25%** | **25%** | 35% | 15% | 15% | 10% |
| 레버리지 | 0% | 0% | 0% | 0% | 10% | 0% | 0% | 0% |
| 금 | **13%** | **24%** | **28%** | 19% | 20% | 25% | **33%** | **37%** |
| 은 | 10% | **6%** | **1%** | **5%** | 5% | 0% | **5%** | **5%** |
| 구리 | 6% | **5%** | 5% | 5% | 5% | 0% | 5% | 3% |
| 코스피 | 18% | **10%** | **8%** | 11% | 5% | 5% | 8% | 7% |
| 신흥국 | **13%** | **5%** | **0%** | **5%** | 5% | 5% | 7% | 7% |

**CAUTION 12차 재선정 (2026-04-18, sweep N=30 walk-forward Yahoo 기반 신규 derived 3종 반영):**
- nasdaq 21→25 (+4), gold 25→28 (+3), silver 4→1 (-3), korea 10→8 (-2), emerging 2→0 (-2)
- α 지표: Composed 10Y +39.48%p, TRAIN +3.32%p, TEST +21.01%p, alpha_decay=-17.69%p (과적합 없음)
- 영상 정합 강화: silver↓(video2 "경기둔화=은 하락"), gold↑(video2 구조 헷지),
  emerging↓(stt_kospi FX 악화), nasdaq↑(신규 derived 방어장치 덕 공격성 확보)
- NEUTRAL 은 sweep 결과 gold 24→20 이 video2 "중앙은행 구조 매수" 수동 조정과
  역방향이라 이번에는 보류 (11차 값 유지).

**가이드라인 (분산 원칙, video4/notion 정성 정합, envelope 미강제):**
- RISK_ON 한국+신흥국 합산 ≤ 30 권고 (stt_kospi: 둘 다 DXY/FX 민감 = 하나의 risk factor,
  video4 "한 방향 올인 금지"). 현재 RISK_ON = 18 + 13 = 31% 로 경계선.
- NEUTRAL 위험자산(nasdaq + korea + emerging) ≤ 55 권고 (현재 38+10+5 = 53%).

**11차 envelope 수동 시정 (2026-04, RISK_ON/CORRECTION):**
sweep BLOCKED 레짐(활성일 100 미만)의 envelope 위반을 영상 근거로 수동 조정.
- **RISK_ON**: emerging 19→13 (envelope `emerging ≤ 15` 위반 해소), 여유분 gold 7→13 이관
  (video2 "중앙은행 구조 매수 대세 상승" 정합, RISK_ON 이라도 구조 헷지 강화)
- **CORRECTION**: cash 13→30 (envelope `cash ≥ 30` + video5_analysis §3.3 "숨고르기 30~40%"),
  silver 8→5 (envelope `silver ≤ 5` + video2 "경기둔화 = 은 하락"),
  nasdaq 35→25 및 emerging 9→5 로 방어 재편 (stt_kospi "FX 악화 외인 이탈" 정합)

**NEUTRAL/CAUTION 11차 재선정 (2026-04, top-decile N=30 --walk-forward --tx-cost=5bp):**
CANDIDATE 통과 2개 레짐(활성일 ≥ 100 AND envelopeViolations = 0)만 교체.
- TRAIN(2016-2023) α=+0.66%p, TEST(2024-2026) α=+21.82%p, alpha_decay=-21.16%p (과적합 경고 없음)
- 기존 CAUTION.cash=28 이 신규 envelope (video5_analysis cash ≥ 30) 위반이었던 점 해소
- NEUTRAL: gold 14→24 (video2 구조헷지), neutral 기조 유지하되 korea/emerging 축소
- CAUTION: nasdaq 29→21, silver 7→4 (경기둔화 국면 방어 강화, video2 정합)
- 나머지 4개(RISK_ON/CORRECTION/PANIC/RECESSION) 는 저표본 BLOCKED → 현 값 유지
...
| **silver ≤ 5** | **CAUTION/CORRECTION/PANIC/RECESSION/STAGFLATION/BOND_VIGILANTE** | video2 "은 아웃퍼폼 = 경기 회복 신호"; 2008 금융위기 금 -30% vs 은 -50%. **경기확장 아닌 6개 국면 모두 상한 (11차 2차 확장: STAGFLATION 성장↓/BOND_VIGILANTE 재정·금리 경제 불안 포함)** |
